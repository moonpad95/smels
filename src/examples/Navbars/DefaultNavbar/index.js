...
import logo1 from "assets/images/logos/gray-logos/logols3.png"
...
function DefaultNavbar({ routes, transparent, light, action, sticky, relative, center }) {
  ...
  const renderRoutes = routes.map(({ name, collapse, columns, rowsPerColumn }) => {
    ...
    if (collapse && columns && name === dropdownName) {
      ...
      template = (
        <Grid key={name} container spacing={1} py={1} px={1.5}>
  ...
  // BARRA DE NAVEGACION EN SU FORMA ACTUAL
  return (
    <Container sx={sticky ? { position: "sticky", top: 0, zIndex: 10 } : null}>
      ...
      >
        <MKBox display="flex" justifyContent="space-between" alignItems="center" p={1.5}>
          <MKBox
            component={Link}
            to="/"
            lineHeight={1}
            py={transparent ? 1.5 : 0}
            pl={relative || transparent ? 0 : { xs: 0, lg: 12 }}
          >
            <img src={logo1} alt="Servicio Médico Empresarial LS" width={90} />
          </MKBox>
          <MKBox
            color="inherit"
...
DefaultNavbar.defaultProps = {
  brand: "Servicio Médico Empresarial L.S.",
  transparent: false,
  light: false,
  action: false,
  sticky: false,
